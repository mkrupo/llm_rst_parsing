...
The normalization algorithm was imported from the author's ``sfb_retreat_26``
repository at commit ``aeff129def1cde9aa94655be772c9da8432a963a``.  Local
changes are limited to fail-closed file handling and command-line ergonomics.
"""
...
def write_normalized_rs3(source, output, *, strict=True, overwrite=False) -> None:
    """Normalize one document and write it without replacing files by default."""
    output_path = Path(output)
    if isinstance(source, (str, Path)):
        source_path = Path(source)
        if source_path.resolve() == output_path.resolve():
            raise FileExistsError("source and normalized output must be different files")
    payload = rs3_bytes(normalize_rs3(source, strict=strict))
    mode = "wb" if overwrite else "xb"
    with output_path.open(mode) as output_file:
        output_file.write(payload)


def _normalization_jobs(in_path: Path, out_path: Path):
    if in_path.is_file():
        if in_path.suffix.lower() != ".rs3":
            raise ValueError("input file must have an .rs3 extension")
        if out_path.exists() and out_path.is_dir():
            return [(in_path, out_path / in_path.name)]
        return [(in_path, out_path)]
    if not in_path.is_dir():
        raise ValueError("input must be an existing .rs3 file or directory")
    inputs = sorted(in_path.glob("*.rs3"))
    if not inputs:
        raise ValueError("input directory contains no .rs3 files")
    if out_path.exists() and not out_path.is_dir():
        raise ValueError("directory input requires a directory output")
    return [(input_path, out_path / input_path.name) for input_path in inputs]


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate and canonically normalize one RS3 file or a flat directory "
            "of RS3 files"
        )
    )
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="replace existing derived outputs (never permits input=output)",
    )
    args = parser.parse_args(argv)
    try:
        jobs = _normalization_jobs(args.in_path, args.out_path)
        for input_path, output_path in jobs:
            if input_path.resolve() == output_path.resolve():
                raise ValueError(
                    f"source and normalized output are the same file: {input_path}"
                )
            if output_path.exists() and not args.overwrite:
                raise FileExistsError(f"refusing to replace existing output: {output_path}")

        # Validate and serialize the entire batch before creating any output.
        payloads = [
            (
                output_path,
                rs3_bytes(normalize_rs3(input_path, strict=not args.compatibility)),
            )
            for input_path, output_path in jobs
        ]
    except (FileExistsError, RS3SchemaError, RS3StructureError, ValueError) as error:
        parser.error(str(error))

    if args.in_path.is_dir() or (args.out_path.exists() and args.out_path.is_dir()):
    ...
    for output_path, payload in payloads:
        ...
        with output_path.open(mode) as output_file:
            output_file.write(payload)
    print(f"normalized={len(payloads)}")
    return 0
